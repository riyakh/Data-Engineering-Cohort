...
class DBAcademyHelper(NestedNamespace):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            spark.sql(f"CREATE SCHEMA IF NOT EXISTS {default_catalog}.ops")
            spark.sql(f"""CREATE OR REPLACE TABLE {default_catalog}.ops.meta(
                owner STRING,
                object STRING,
                key STRING,
                value STRING)""")
            spark.sql(f"""INSERT INTO {default_catalog}.ops.meta (owner, object, key, value)
            SELECT current_user(), 'account users', 'datasets.wine_quality', 'dbacademy_wine_quality.v01' UNION ALL
            SELECT current_user(), 'account users', 'paths.datasets.wine_quality', '/Volumes/dbacademy_wine_quality/v01' UNION ALL
            SELECT current_user(), NULL, 'username', current_user() UNION ALL
            SELECT current_user(), NULL, 'catalog_name', '{default_catalog}' UNION ALL
            SELECT current_user(), NULL, 'schema_name', regexp_replace(regexp_replace(current_user(), '@.*', ''), '[.]', '_') UNION ALL
            SELECT current_user(), NULL, 'paths.working_dir',
                concat('/Volumes/{default_catalog}/ops/',regexp_replace(regexp_replace(current_user(), '@.*', ''), '[.]', '_')) UNION ALL
            SELECT current_user(), NULL, 'cluster_name', regexp_replace(current_user(), '@.*', '') UNION ALL
            SELECT current_user(), NULL, 'pseudonym', regexp_replace(current_user(), '@.*', '');
            """)
            username = spark.sql("SELECT current_user()").collect()[0][0]
            username_clean= username.split("@")[0].replace(".", "_")
            schema = username.replace('@.*', '').split('@')[0]
            spark.sql(f"CREATE SCHEMA IF NOT EXISTS {default_catalog}.{schema}")
            spark.sql(f"CREATE VOLUME IF NOT EXISTS {default_catalog}.ops.{username_clean}")
        except:
